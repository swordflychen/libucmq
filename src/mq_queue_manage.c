...
bool mq_qm_open_store(void)
{
    ...
    while ((ent = readdir(p_dir)) != NULL)
    {
        ...
        if (!mq_sm_wtag_open_file(&tmp_queue.mq_queue, &queue_file))
        {
            log_warn("Can't open rtag file [%s]", queue_file.wtag_fname);
            continue;
        }
        ...
        if (!mq_sm_wtag_read_item(&tmp_queue.mq_queue))
        {
            log_warn("Can't read wtag [%s]'s item", queue_file.wtag_fname);
            continue;
        }
...
bool mq_qm_close_store(void)
{
    ...
    for(tmp_queue = g_mq_qlist; tmp_queue != NULL; tmp_queue = tmp_queue ->hh.next) 
    {
        ...
        if (!mq_sm_wtag_close_file(&tmp_queue->mq_queue))
        {
            log_warn( "Close queue [%s] wtage file fail",
                    tmp_queue->qname);
        }
...
int mq_qm_push_item(mq_queue_t *mq_queue, msg_item_t *msg_item)
{
    ...
    {
        if (mq_queue->put_num - mq_queue->get_num + 1 > mq_queue->maxque)
        {
            log_debug("Put error, uread [%d], Is full", mq_queue->put_num - mq_queue->get_num);
            return QUEUE_FULL;
        }
    }
    ...
    if (!mq_sm_wtag_write_item(mq_queue))
    {
        log_warn("Queue [%s] wtag sync ... fail", mq_queue->qname);
    }
...
//    if (mq_queue->unread_count < 1)
    if (mq_queue->put_num - mq_queue->get_num < 1)
    {
...
static bool init_queue(mq_queue_t *mq_queue)
{
    ...
    pthread_mutex_init(&(mq_queue->mutex), NULL);
