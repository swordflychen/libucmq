...
bool get_next_write_file(mq_queue_t *mq_queue)
{
    ...
    int         old_wtag_file_fd;
    char        new_wtag_fname[MAX_FULL_FILE_NAME_LEN + 1];
    char        old_wtag_fname[MAX_FULL_FILE_NAME_LEN + 1];
    ...
    /* Current file name */
    old_wtag_file_fd = mq_queue->wtag_fd;
    GEN_WTAG_FULL_PATH_BY_QNAME_INDEX(old_wtag_fname, mq_queue->qname, mq_queue->cur_wdb.cur_index);
    GEN_WTAG_FULL_PATH_BY_QNAME_INDEX(new_wtag_fname, mq_queue->qname, next_index);
    ...
    /* 1. Open the next rtag file and remove old wtag file */
    if((mq_queue->wtag_fd = mq_sm_wtag_open_next_file(new_wtag_fname)) < 0)
    {
        log_warn("Get new wtag file fail, errno[%d], errno msg[%s]",
                errno, strerror(errno));
        return false;
    }
    ...
    if((mq_queue->cur_wdb.map_mem
            ...
            == NULL)
    {
        log_warn("Open write data file[%s] fail", new_db_fname);
        return false;
    }
    ...
    close(old_wtag_file_fd);
    unlink(old_wtag_fname);
...
bool find_handle_file(queue_file_t *queue_file, char *qname)
{
    ...
    char   wtag_file_pattern[MAX_DATA_FILE_NAME_LEN + 1];
    ...
    snprintf(wtag_file_pattern, MAX_DATA_FILE_NAME_LEN,"%s_", "wtag");
    db_pattern_len   = strlen(db_file_pattern);
    rtag_pattern_len = strlen(rtag_file_pattern);
    wtag_pattern_len = strlen(wtag_file_pattern);
    ...
    while ((ent = readdir(p_dir)) != NULL)
    {
        ...
        /* wtag file find */
        if (strncmp(ent->d_name, wtag_file_pattern, wtag_pattern_len) == 0)
        {
            if (k == 1)
            {
                strcpy(queue_file->wtag_fname, ent->d_name);
            }
            else
            {
                /* If wtag file is empty remove it */
                if (get_file_size(ent->d_name) < 0)
                {
                    unlink(ent->d_name);
                    continue;
                }
                strcmp(ent->d_name, queue_file->wtag_fname) < 0 ? unlink(ent->d_name) : unlink(queue_file->wtag_fname);

                /* Note new rtag file */
                strcpy(queue_file->wtag_fname,
                        strcmp(ent->d_name, queue_file->wtag_fname) > 0 ? ent->d_name : queue_file->wtag_fname);
                log_debug( "Temp wtag file name[%s]", queue_file->wtag_fname);
            }
            k++;
        }
    }//while
    ...
    if (queue_file->wtag_fid != queue_file->write_fid)
    {
        log_error("Wtag file index  doesn't match write file index");
        return false;
    }
